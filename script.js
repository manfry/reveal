const CONFIG = {
  ...
  revealDate: "2026-09-09T20:15:00",
  // "male" oppure "female"
  gender: "female",
  texts: {
...
function launchConfetti(gender) {
  ...
  for (let i = 0; i < 180; i++) {
    ...
    pieces.push({
      x: innerWidth / 2, y: innerHeight / 2, w: 5 + Math.random() * 8, h: 8 + Math.random() * 14,
  ...
  for (let i = 0; i < 100; i++) {
    pieces.push({
      x: Math.random() * innerWidth, y: -Math.random() * innerHeight, w: 5 + Math.random() * 8, h: 8 + Math.random() * 14,
